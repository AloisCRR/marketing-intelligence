...
- **Run 2 (bootstrap `sabrikolod`, `resultsLimit: 10`):** PASS with note. 10 posts returned but recency-ish, not strictly newest (mix of 2025-11 → 2026-09; 3 hidden-like `-1` rows). 4/10 carry `ChismecitoMarketinero` → client-side filter works. Bootstrap is a seed, not an ordering guarantee.
